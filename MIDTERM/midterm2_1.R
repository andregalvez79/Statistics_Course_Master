...
#multicollinearity
vif(step3)
#there is no multicollinearity, VIF is lower than 10 for every variable.
1/vif(step3)
#there is no multicollinearity, VIF tolerance says that below.1 there is a serious problem and all values are higher than .4.

#linearity
qqnorm(regress_1$resid)
...
scatter + geom_point() + geom_smooth(method = "loess", colour = "Blue")+ labs(x = "Fitted Values", y = "Standardize Residual")
...
library(ggplot2)
plotint <- ggplot(regress_1, aes(x=ANXIETY, y=FEAR, color=gender, shape=gender)) 
plotint <- plotint + geom_point() + geom_smooth(method=lm, se=FALSE, fullrange=TRUE, size=I(1.3)) + scale_color_manual(values=c('#999999','black'))
plotint
